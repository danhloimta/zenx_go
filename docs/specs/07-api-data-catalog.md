...
| ---------------------------- | ----------------------------- | --------------------------- | ------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| `API-AUTH-REGISTER`          | `POST /auth/register`         | Public                      | `IMPLEMENTED` | `RegisterDto`; phone token bắt buộc tùy `otpRequired`, accept terms/privacy; trả public user + cookies. | `apps/api/src/auth/auth.controller.ts`; vertical/account OTP integration |
...
| `API-AUTH-FORGOT`            | `POST /auth/forgot-password`  | Public                      | `IMPLEMENTED` | Email input; gửi reset OTP chỉ khi `otpRequired=true`, response accepted không lộ account existence. | auth service; vertical/account OTP integration |
| `API-AUTH-RESET`             | `POST /auth/reset-password`   | Public + optional verification token | `IMPLEMENTED` | Email/token tùy policy/new password; reset password và revoke sessions.              | auth service; account OTP integration                        |
| `API-AUTH-ME`                | `GET /auth/me`                | Access                      | `IMPLEMENTED` | Trả userId/username từ access session.                                              | auth controller; vertical integration                        |
| `API-AUTH-PROVIDER-AVAILABILITY` | `GET /auth/provider-availability` | Public                  | `IMPLEMENTED` | Fresh DB projection `{ google, facebook, otpRequired, phoneRegistrationOtpRequired }`; `Cache-Control: no-store`; fail closed `503 SETTINGS_UNAVAILABLE`. | auth-settings service/controller; auth-settings integration; provider-availability E2E |
| `API-AUTH-GOOGLE-START`      | `GET /auth/google`            | Public hoặc link session    | `PARTIAL`     | OAuth mode/returnTo; mode `login` reads settings; disabled/unavailable redirects `302` with `social_error=provider_disabled` / `social_error=settings_unavailable`; mode `link` unchanged; otherwise redirect provider hoặc `not_configured`. | auth/settings/social services; auth-settings integration; social/provider E2E |
| `API-AUTH-GOOGLE-CALLBACK`   | `GET /auth/google/callback`   | OAuth state                 | `PARTIAL`     | Verify state; mode `login` re-reads settings before exchange; disabled/unavailable redirects `302` with lowercase `social_error=provider_disabled` / `social_error=settings_unavailable`; link path unchanged. | auth/settings/social services; auth-settings integration; social/provider E2E |
| `API-AUTH-FACEBOOK-START`    | `GET /auth/facebook`          | Public hoặc link session    | `PARTIAL`     | OAuth mode/returnTo; mode `login` reads settings; disabled/unavailable redirects `302` with `social_error=provider_disabled` / `social_error=settings_unavailable`; mode `link` unchanged; otherwise redirect provider hoặc `not_configured`. | auth/settings/social services; auth-settings integration; social/provider E2E |
| `API-AUTH-FACEBOOK-CALLBACK` | `GET /auth/facebook/callback` | OAuth state                 | `PARTIAL`     | Verify state; mode `login` re-reads settings before exchange; disabled/unavailable redirects `302` with lowercase `social_error=provider_disabled` / `social_error=settings_unavailable`; link path unchanged. | auth/settings/social services; auth-settings integration; social/provider E2E |
...
| `API-ACCOUNT-PASSWORD`             | `POST /account/change-password`              | Access                   | `IMPLEMENTED` | Current/new password + optional `CHANGE_PASSWORD` token; policy-gated OTP, strength/reuse check, revoke sessions. | account service; account OTP integration |
...
| `API-GAME-BY-SUBDOMAIN` | `GET /games/by-subdomain/:subdomain`     | Public | `IMPLEMENTED` | Public game detail/config; projects safe maintenance notice and hides SSO during maintenance/unavailable status. | game service; game/portal integration     |
| `API-GAME-SSO-AUTHORIZE` | `GET /game-sso/authorize` | Browser session | `IMPLEMENTED` | Validates active client/callback and redirects with a one-time authorization code; returns `GAME_SSO_UNAVAILABLE` for maintenance/unavailable games. | game-sso service/controller |
| `API-GAME-SSO-EXCHANGE` | `POST /game-sso/exchange` | HTTP Basic game client | `IMPLEMENTED` | Consumes a 60-second code once and returns minimal identity while recording game player login; rechecks maintenance/unavailable status. | game-sso service/controller |
| `API-ADMIN-GAME-ACCESS` | `GET/PUT /admin/games/:gameId/admins[/userId]` | `SUPER_ADMIN` | `IMPLEMENTED` | Lists and replaces one user's fixed-game role set; reason is required and role changes are audited. | game-management service/controller; multi-game integration, `game-admin.spec.ts` |
| `API-ADMIN-GAME-SSO` | `GET/PATCH /admin/games/:gameId/sso-client`, `POST …/rotate-secret` | `SUPER_ADMIN` | `IMPLEMENTED` | Creates/configures active client or rotates one secret; read response never contains secret/hash. | game-management service/controller; multi-game integration, `game-admin.spec.ts` |
| `API-GAME-ADMIN-PLAYERS` | `GET /game-admin/games/:gameId/players`, `GET /players/:userId`, `PATCH /players/:userId/status` | Player Moderator / Game Admin / `SUPER_ADMIN` | `IMPLEMENTED` | Scoped player search/paging, profile and per-game block state; response omits email, phone and wallet. | game-admin service/controller |
| `API-GAME-ADMIN-PLAYER-SUPPORT` | `PATCH /game-admin/games/:gameId/players/:userId/support-note`, `GET …/activity` | Player Moderator / Game Admin / `SUPER_ADMIN` | `IMPLEMENTED` | Saves one 2,000-character internal note with optimistic concurrency and returns only that player's block/unblock/note activity. | game-admin service/controller; multi-game integration |
| `API-GAME-ADMIN-PLAYER-PROFILE` | `GET/PATCH /game-admin/games/:gameId/players/:userId/profile` | Game Admin / `SUPER_ADMIN` | `IMPLEMENTED` | Reads/updates the existing global ZENX basic profile only after game-player scope validation; contact verification flags are explicit and safe audit is written in the same transaction. | game-admin/admin service; multi-game integration |
| `API-GAME-ADMIN-OPERATIONS` | `GET /game-admin/games/:gameId/operations`, `PATCH …/operations/maintenance` | Game Admin / `SUPER_ADMIN` | `IMPLEMENTED` | Transitions only `AVAILABLE ↔ MAINTENANCE`, validates reason/message/time, writes safe audit metadata and returns stale/locked conflicts. | game-admin service/controller; multi-game integration |
| `API-GAME-ADMIN-CONTENT` | `GET/POST/PATCH/DELETE /game-admin/games/:gameId/{articles,events,presentation}` | Game Content Manager / `SUPER_ADMIN` | `IMPLEMENTED` | Fixed-game CMS reads/mutations; article restore is `POST articles/:articleId/restore`; resource ownership and optimistic concurrency are enforced. | game-content service/controller; multi-game integration, `game-content-admin.spec.ts` |
| `API-GAME-ADMIN-AUDIT` | `GET /game-admin/games/:gameId/audit` | Game Admin / `SUPER_ADMIN` | `IMPLEMENTED` | Game-scoped audit filters action/actor/date and returns safe before/after metadata only. | game-admin service/controller; multi-game integration |
...
| `API-ADMIN-AUTH-SETTINGS-GET` | `GET /admin/settings/auth-providers`                 | Access + `settings.auth.manage` | `IMPLEMENTED` | Fresh singleton read; Google/Facebook + canonical `otpRequired` (legacy alias preserved) + ISO `updatedAt`; `503 SETTINGS_UNAVAILABLE` on missing row/DB failure. | auth-settings controller/service; auth-settings integration |
| `API-ADMIN-AUTH-SETTINGS-PATCH` | `PATCH /admin/settings/auth-providers`             | Access + `settings.auth.manage` | `IMPLEMENTED` | `expectedUpdatedAt` + at least one Google/Facebook/OTP boolean; canonical `otpRequired` or legacy alias; optimistic update; `409 STALE_AUTH_SETTINGS_UPDATE`, `503 SETTINGS_UNAVAILABLE`. | auth-settings controller/service; auth-settings integration |
...
| `AuthSettings` / `auth_settings`                                  | Singleton `id=1`; Google/Facebook login-registration và physical legacy column `phone_registration_otp_required` default `true` cùng `updatedAt`. | Public/admin chuẩn dùng `otpRequired`, vẫn trả/nhận alias `phoneRegistrationOtpRequired`; không lưu OAuth secrets/config. |
...
| `UserActivityLog` / `user_activity_logs`                          | Login/security event, result, IP, device label, raw User-Agent và metadata allowlist; retention 180 ngày.      | Player chỉ đọc log của mình; Admin `users.view` đọc theo player, gồm raw User-Agent. |

...
| ---------------------------- | ----------------------------- | --------------------------- | ------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| `API-AUTH-REGISTER`          | `POST /auth/register`         | Public                      | `IMPLEMENTED` | `RegisterDto`; token phone bắt buộc tùy `phoneRegistrationOtpRequired`, accept terms/privacy; trả public user + cookies. | `apps/api/src/auth/auth.controller.ts`; vertical integration |
...
| `API-AUTH-PROVIDER-AVAILABILITY` | `GET /auth/provider-availability` | Public                  | `IMPLEMENTED` | Fresh DB projection `{ google, facebook, phoneRegistrationOtpRequired }`; `Cache-Control: no-store`; fail closed `503 SETTINGS_UNAVAILABLE`. | auth-settings service/controller; auth-settings integration; provider-availability E2E |
...
| `API-ADMIN-AUTH-SETTINGS-GET` | `GET /admin/settings/auth-providers`                 | Access + `settings.auth.manage` | `IMPLEMENTED` | Fresh singleton read; Google/Facebook/phone-OTP booleans + ISO `updatedAt`; `503 SETTINGS_UNAVAILABLE` on missing row/DB failure. | auth-settings controller/service; auth-settings integration |
| `API-ADMIN-AUTH-SETTINGS-PATCH` | `PATCH /admin/settings/auth-providers`             | Access + `settings.auth.manage` | `IMPLEMENTED` | `expectedUpdatedAt` + ít nhất một trong ba boolean; optimistic update; `409 STALE_AUTH_SETTINGS_UPDATE`, `503 SETTINGS_UNAVAILABLE`. | auth-settings controller/service; auth-settings integration |
...
| `AuthSettings` / `auth_settings`                                  | Singleton `id=1`; Google/Facebook login-registration và `phoneRegistrationOtpRequired` booleans default `true` cùng `updatedAt`. | Public nhận `{ google, facebook, phoneRegistrationOtpRequired }`; admin nhận ba field đầy đủ + `updatedAt`; không lưu OAuth secrets/config. |

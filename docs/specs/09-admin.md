...
- `GAME_ADMIN` may use `game.players.profile.manage` to update the existing global ZENX basic profile of a player scoped to that game: username, email, phone, full name, date of birth, gender, city, address and contact verification flags. Username/email/phone changes remain global and revoke active sessions; wallet, roles, passwords, account status and sensitive identity remain Command Hub-only. A non-Super-Admin cannot edit a player who holds any platform or game-admin assignment.

...
- A game in `MAINTENANCE` or `UNAVAILABLE` does not expose an SSO launch URL. Both authorize and exchange return `503 GAME_SSO_UNAVAILABLE`; a code issued before maintenance cannot be exchanged until the game is available again. Existing game-server sessions are not revoked.
- Public game data projects maintenance as `{ message, expectedEndsAt }` only while the operational status is `MAINTENANCE`. The site stays readable and renders this notice while hiding **Chơi ngay**.
...
- Browser: `apps/web/e2e/account-screens.spec.ts`, `auth-subdomain.spec.ts`, `auth-provider-availability.spec.ts`, `game-admin.spec.ts` (callback/state, exchange, player-support and maintenance lifecycle).

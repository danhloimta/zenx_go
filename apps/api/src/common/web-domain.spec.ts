...
describe('web domain policy', () => {
  ...
  it('recognizes only direct game subdomains and reserved names', () => {
    ...
    expect(classifyWebHost('www.zenxgo.io.vn', 'zenxgo.io.vn')).toEqual({ kind: 'WWW', subdomain: 'www' });
  ...
  it('allows explicit origins and game subdomains but rejects foreign origins', () => {
    ...
    expect(isAllowedWebOrigin('http://zenxgo.io.vn', 'zenxgo.io.vn', [], true, true)).toBe(false);
    expect(isAllowedWebOrigin('http://lvh.me:3000', 'lvh.me', ['http://lvh.me:3000'], true, true)).toBe(false);
  });
  ...
  it('accepts only absolute return URLs within the ecosystem', () => {
    ...
    expect(isAllowedReturnTo('http://lucdia.zenxgo.io.vn/tin-tuc', 'zenxgo.io.vn', [], true, true)).toBe(false);
    expect(isAllowedReturnTo('https://api.zenxgo.io.vn/account', 'zenxgo.io.vn')).toBe(false);
    expect(isAllowedReturnTo('https://a.lucdia.zenxgo.io.vn/account', 'zenxgo.io.vn')).toBe(false);
  });
});

...
export class GameContentService {
  ...
  async updateEvent(gameId: string, eventId: string, dto: AdminContentEventUpdateDto, actorUserId: string) { await this.event(gameId, eventId); const before = await this.content.getEvent(eventId); const safeDto = { ...dto }; delete safeDto.gameId; const result = await this.content.updateEvent(eventId, safeDto); await this.audit(actorUserId, gameId, 'GAME_EVENT_UPDATED', 'GAME_EVENT', eventId, before, result); return result; }

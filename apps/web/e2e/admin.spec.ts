...
test('admin dashboard manages a user account and records activity', async ({ page, request }) => {
  ...
  await page.getByLabel('Tìm người dùng').fill(memberEmail);
  ...
  await page.getByLabel('Lý do cập nhật').fill('Cập nhật thông tin theo yêu cầu');
  await page.getByLabel('Họ và tên').fill('E2E Admin Updated');
  await page.getByRole('button', { name: 'Lưu thay đổi', exact: true }).click();
  await expect(page.getByText('Đã cập nhật hồ sơ người dùng.')).toBeVisible();

  await page.getByRole('button', { name: 'Tạm ngưng', exact: true }).click();
  await page.getByRole('textbox', { name: 'Lý do', exact: true }).fill('Tạm ngưng để kiểm tra');
  ...
  await page.getByRole('textbox', { name: 'Lý do', exact: true }).fill('Đã hoàn tất kiểm tra');
  await page.getByRole('button', { name: 'Xác nhận', exact: true }).click();
  await expect(page.getByText('Đang hoạt động', { exact: true }).first()).toBeVisible();

  await page.getByRole('button', { name: 'Đặt mật khẩu tạm', exact: true }).click();
  await page
    .getByRole('textbox', { name: 'Mật khẩu tạm', exact: true })
    .fill('TemporaryPassword123!');
  await page
    .getByRole('textbox', { name: 'Xác nhận mật khẩu tạm', exact: true })
    .fill('TemporaryPassword123!');
  await page.getByRole('textbox', { name: 'Lý do', exact: true }).fill('Cấp lại mật khẩu cho user');
  await page.getByRole('button', { name: 'Xác nhận', exact: true }).click();
  await expect(page.getByText('Đã đặt mật khẩu tạm và thu hồi các phiên cũ.')).toBeVisible();

  await page.goto('/admin/audit-logs');
  await expect(page.getByText('Đổi trạng thái tài khoản', { exact: true }).first()).toBeVisible();
});

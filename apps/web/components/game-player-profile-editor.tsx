...
export function GamePlayerProfileEditor({ gameId, userId }: { gameId: string; userId: string }) {
  ...
  const update = useMutation({
    ...
    onError: (cause) => setError(getErrorMessage(cause, 'Không thể cập nhật hồ sơ. Nội dung đang nhập vẫn được giữ lại.')),
  });
  ...
  if (profile.isError || !profile.data) return <section className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm text-red-700">Không thể tải hồ sơ đầy đủ của player.</section>;
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-5">
    ...
    <div className="mt-5 space-y-5">
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Tên đăng nhập"><Input value={form.username} onChange={(event) => { setForm({ ...form, username: event.target.value }); setError(null); }} /></Field>
        <Field label="Email"><Input type="email" value={form.email} onChange={(event) => { setForm({ ...form, email: event.target.value }); setError(null); }} /><Check label="Email đã xác minh" checked={form.emailVerified} onChange={(emailVerified) => setForm({ ...form, emailVerified })} /></Field>
        <Field label="Số điện thoại" className="md:col-span-2"><Input value={form.phone} onChange={(event) => { setForm({ ...form, phone: event.target.value }); setError(null); }} placeholder="Chưa cập nhật số điện thoại" /><Check label="Số điện thoại đã xác minh" checked={form.phoneVerified} onChange={(phoneVerified) => setForm({ ...form, phoneVerified })} /></Field>
      </div>
      <div className="grid gap-4 border-t border-slate-100 pt-5 md:grid-cols-2">
        <Field label="Họ và tên"><Input value={form.fullName} onChange={(event) => { setForm({ ...form, fullName: event.target.value }); setError(null); }} /></Field>
        <Field label="Ngày sinh"><Input type="date" value={form.dateOfBirth} onChange={(event) => { setForm({ ...form, dateOfBirth: event.target.value }); setError(null); }} /></Field>
        <Field label="Giới tính"><Select value={form.gender} onChange={(event) => { setForm({ ...form, gender: event.target.value as Gender }); setError(null); }}><option value="UNSPECIFIED">Chưa xác định</option><option value="MALE">Nam</option><option value="FEMALE">Nữ</option><option value="OTHER">Khác</option></Select></Field>
        <Field label="Tỉnh / thành phố"><Input value={form.city} onChange={(event) => { setForm({ ...form, city: event.target.value }); setError(null); }} /></Field>
        <Field label="Địa chỉ" className="md:col-span-2"><Input value={form.address} onChange={(event) => { setForm({ ...form, address: event.target.value }); setError(null); }} /></Field>
      </div>
    </div>

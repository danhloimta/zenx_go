...
import { LayoutDashboard, Users, ShieldCheck, FileText, CalendarDays, Palette, History } from 'lucide-react';
...
export function GameAdminShell({ children }: { children: React.ReactNode }) {
  ...
  const items = [{ href: `/game-admin/${subdomain}`, label: 'Tổng quan', icon: LayoutDashboard }]
    .concat(can('manage', 'GamePresentation') ? [{ href: `/game-admin/${subdomain}/presentation`, label: 'Giao diện', icon: Palette }] : [])
    .concat(can('manage', 'GameContent') ? [{ href: `/game-admin/${subdomain}/articles`, label: 'Bài viết', icon: FileText }] : [])
    .concat(can('manage', 'GameEvent') ? [{ href: `/game-admin/${subdomain}/events`, label: 'Sự kiện', icon: CalendarDays }] : [])
    .concat(can('read', 'GamePlayer') ? [{ href: `/game-admin/${subdomain}/players`, label: 'Người chơi', icon: Users }] : [])
    .concat(can('read', 'GameAudit') ? [{ href: `/game-admin/${subdomain}/audit`, label: 'Nhật ký hoạt động', icon: History }] : []);

...
export function GamePresentationEditor({ value, preset, onChange }: { value: PresentationValue; preset: GameTemplatePreset; onChange: (value: PresentationValue) => void }) {
  ...
  return <div className="space-y-5">
    ...
    {preset === 'SCI_FI_SHOOTER' ? <><ItemListEditor label="Vai trò" items={value.pageConfig.roles} onChange={(items) => setItems('roles', items)} /><ItemListEditor label="Khu vực" items={value.pageConfig.locations} onChange={(items) => setItems('locations', items)} /><ItemListEditor label="Trang bị" items={value.pageConfig.equipment} onChange={(items) => setItems('equipment', items)} /></> : null}

...
type Form = { code: string; title: string; message: string; ctaLabel: string; ctaPath: string; status: ContentPublishStatus; startsAt: string; endsAt: string; sortOrder: string };
...
function toForm(item: AdminContentAnnouncement | null): Form { return item ? { code: item.code, title: item.title, message: item.message, ctaLabel: item.ctaLabel ?? '', ctaPath: item.ctaPath ?? '', status: item.status, startsAt: toInput(item.startsAt), endsAt: toInput(item.endsAt), sortOrder: String(item.sortOrder) } : { code: '', title: '', message: '', ctaLabel: '', ctaPath: '', status: 'DRAFT', startsAt: '', endsAt: '', sortOrder: '0' }; }
function toCreate(form: Form) { return { code: form.code, title: form.title, message: form.message, ctaLabel: form.ctaLabel || null, ctaPath: form.ctaPath || null, status: form.status, startsAt: toApi(form.startsAt), endsAt: form.endsAt ? toApi(form.endsAt) : null, sortOrder: Number(form.sortOrder) || 0 }; }
function toUpdate(item: AdminContentAnnouncement, form: Form) { return { title: form.title, message: form.message, ctaLabel: form.ctaLabel || null, ctaPath: form.ctaPath || null, status: form.status, startsAt: toApi(form.startsAt), endsAt: form.endsAt ? toApi(form.endsAt) : null, sortOrder: Number(form.sortOrder) || 0, expectedUpdatedAt: item.updatedAt }; }
...
function AnnouncementDialog({ item, pending, onClose, onSubmit }: { item: AdminContentAnnouncement | null; pending: boolean; onClose: () => void; onSubmit: (form: Form) => void }) {
  ...
  const canSubmit = form.title.trim().length >= 2 && form.message.trim().length > 0 && Boolean(form.startsAt) && (item || form.code.trim().length >= 2);
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><h2 className="text-lg font-black text-slate-900">{item ? 'Sửa thông báo' : 'Tạo thông báo'}</h2><p className="mt-1 text-xs text-slate-500">Cập nhật trực tiếp thông tin thông báo.</p></div><button onClick={onClose} aria-label="Đóng" className="rounded-lg p-1 text-slate-400 hover:bg-slate-100"><X className="size-5" /></button></div><div className="mt-6 grid gap-5 md:grid-cols-2"><Field label="Code"><Input value={form.code} onChange={(event) => set('code', event.target.value)} disabled={Boolean(item)} /></Field><Field label="Trạng thái"><Select value={form.status} onChange={(event) => set('status', event.target.value as ContentPublishStatus)}><option value="DRAFT">Bản nháp</option><option value="PUBLISHED">Đã publish</option></Select></Field><Field label="Tiêu đề"><Input value={form.title} onChange={(event) => set('title', event.target.value)} /></Field><Field label="Thứ tự"><Input type="number" value={form.sortOrder} onChange={(event) => set('sortOrder', event.target.value)} /></Field><div className="md:col-span-2"><Field label="Thông điệp"><Textarea value={form.message} onChange={(event) => set('message', event.target.value)} className="min-h-24" /></Field></div><Field label="Bắt đầu"><Input type="datetime-local" value={form.startsAt} onChange={(event) => set('startsAt', event.target.value)} /></Field><Field label="Kết thúc (tuỳ chọn)"><Input type="datetime-local" value={form.endsAt} onChange={(event) => set('endsAt', event.target.value)} /></Field><Field label="CTA label"><Input value={form.ctaLabel} onChange={(event) => set('ctaLabel', event.target.value)} /></Field><Field label="CTA path"><Input value={form.ctaPath} onChange={(event) => set('ctaPath', event.target.value)} /></Field></div><div className="mt-6 flex justify-end gap-2"><Button variant="outline" onClick={onClose}>Huỷ</Button><Button disabled={pending || !canSubmit} onClick={() => onSubmit(form)}><Save className="size-4" /> {pending ? 'Đang lưu…' : 'Lưu thay đổi'}</Button></div></div></div>;
}

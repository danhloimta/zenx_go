...
export default function RoleDetailPage() {
  ...
  const update = useMutation({
    mutationFn: () => api.admin.access.updateRole(roleId, { expectedUpdatedAt: role.data!.updatedAt, name: name.trim(), description: description.trim() || null, isActive, reason: reason.trim() }),
  ...
  const savePermissions = useMutation({
    mutationFn: () => api.admin.access.replacePermissions(roleId, { expectedUpdatedAt: role.data!.updatedAt, permissionIds, reason: reason.trim() }),
    onSuccess: () => { setReason(''); refresh(); toast.success('Đã cập nhật permission và thu hồi phiên liên quan.'); },
    onError: (error) => { toast.error(getErrorMessage(error)); if ((error as { code?: string }).code === 'STALE_ROLE_UPDATE') refresh(); },
  });
  const remove = useMutation({
    mutationFn: () => api.admin.access.deleteRole(roleId),

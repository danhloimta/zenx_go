...
export default function RolesPage() {
  ...
  const roles = useQuery({ queryKey: ['admin', 'access', 'roles'], queryFn: () => api.admin.access.roles(), enabled: canView, retry: false });
  const permissions = useQuery({ queryKey: ['admin', 'access', 'permissions'], queryFn: api.admin.access.permissions, enabled: canView, retry: false });
  const create = useMutation({
    mutationFn: () => api.admin.access.createRole({ code: code.trim().toUpperCase(), name: name.trim(), reason: reason.trim() }),
    onSuccess: () => {
      setCode('');
      setName('');
      setReason('');
  ...
  if (!canView) return <main className="p-8 text-sm text-slate-500">Bạn không có quyền quản lý role.</main>;
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <form className="flex flex-wrap gap-3 rounded-2xl border bg-white p-4" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}>
        ...
        {canCreate ? <><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Lý do tạo role" className="max-w-xs" required /><Button type="submit" disabled={create.isPending || !reason.trim()}>{create.isPending ? 'Đang tạo…' : 'Tạo role'}</Button></> : null}
      </form>

...
export default function SupportTicketsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6 pb-10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 text-sm text-slate-500">
            Theo dõi các vấn đề bạn đã gửi đến đội ngũ ZENX GO.
          </p>
        </div>
        <Button asChild className="w-full gap-2 sm:w-auto">
          <Link href="/support/report-issue">
            <MessageSquarePlus className="size-4" /> Tạo yêu cầu mới
          </Link>
        </Button>
      </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm sm:p-7">
        {query.isLoading ? (
          ...
          <EmptyState
            ...
            description="Nếu gặp vấn đề, hãy gửi yêu cầu để đội ngũ ZENX GO hỗ trợ."
          />
        ) : (
          <>
              ...
              <table className="w-full min-w-[680px] text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-xs font-semibold text-slate-400">
                    <th className="pb-3 pr-4">Mã yêu cầu</th>
                    <th className="px-4 pb-3">Nội dung</th>
                    <th className="px-4 pb-3">Danh mục</th>
                    <th className="px-4 pb-3">Ngày gửi</th>
                    <th className="pb-3 pl-4 text-right">Trạng thái</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {items.map((ticket) => (
                    <tr key={ticket.ticketNo} className="group hover:bg-slate-50">
                      ...
                      <td className="max-w-[300px] px-4 py-4">
                        <Link
                          href={`/account/support/${encodeURIComponent(ticket.ticketNo)}`}
                          className="block truncate font-semibold text-slate-800 group-hover:text-[#00873E]"
                        >
                          {ticket.subject}
                        </Link>
                        <p className="mt-1 truncate text-xs text-slate-400">{ticket.description}</p>
                      </td>
            ...
            <div className="space-y-3 md:hidden">
              {items.map((ticket) => (
                <Link
                  key={ticket.ticketNo}
                  href={`/account/support/${encodeURIComponent(ticket.ticketNo)}`}
                  className="block rounded-xl border border-slate-100 p-4 transition hover:border-[#9CD7AB] hover:bg-[#F9FCFA]"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-[#00873E]">{ticket.ticketNo}</p>
            ...
            <div className="mt-6 flex flex-col gap-3 border-t border-slate-100 pt-4 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
              <span>
                ...
                {Math.min(page * pageSize, query.data?.total ?? 0)} của {query.data?.total ?? 0} yêu
                cầu
              </span>
              <div className="flex items-center gap-2 self-center sm:self-auto">
                <button
                  ...
                  className="flex size-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40"
                >
      ...
      <div className="rounded-2xl border border-[#DDF1E2] bg-[#F0FAF2] p-5 text-sm text-slate-600 sm:p-6">
        <div className="flex items-start gap-3">
          <Ticket className="mt-0.5 size-5 shrink-0 text-[#00873E]" />
          <div>
            <p className="font-bold text-slate-900">Cần tìm câu trả lời nhanh?</p>
            <p className="mt-1">Xem các câu hỏi thường gặp trước khi gửi yêu cầu mới.</p>
            <Link
              href="/support"
              className="mt-2 inline-flex items-center gap-1 font-bold text-[#00873E] hover:underline"
            >
              Xem trung tâm hỗ trợ <ArrowRight className="size-3.5" />
            </Link>
          </div>
        </div>
      </div>

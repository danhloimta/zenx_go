...
function TransactionDetailPanel({ transaction, onCopy }: { transaction: Awaited<ReturnType<typeof api.wallet.transaction>>; onCopy: (text: string) => void }) {
  ...
  return <div className="mt-5 space-y-5">
    ...
    <div className="h-px bg-slate-100" /><div><h3 className="text-xs font-bold text-slate-900">Bạn cần hỗ trợ?</h3><p className="mt-1 text-[11px] text-slate-500 leading-normal">Nếu bạn gặp vấn đề với giao dịch này, vui lòng liên hệ đội ngũ hỗ trợ.</p><Button asChild variant="zenx-outline" className="mt-3 w-full gap-2 text-xs font-semibold h-10 rounded-xl"><a href="mailto:support@zenxgo.vn"><Headphones className="size-3.5" />Liên hệ hỗ trợ</a></Button></div>
  </div>;
}
